...
#
# fetch_utils.py
# Utility functions for fetching data files required by the sfd library.
#
...
from progressbar.widgets import AdaptiveETA
from progressbar.widgets import AdaptiveTransferSpeed
from progressbar.widgets import Bar
from progressbar.widgets import DataSize
from progressbar.widgets import FormatCustomText
...
class FileTransferProgressBar(ProgressBar):
    def __init__(self, content_length):
        ...
        if content_length is None:
            ...
            size_txt = f'{scaled:.1f} {prefixes[power]:s}B'
        widgets = [
            DataSize(),
            FormatCustomText(f' of {size_txt:s} | '),
...
def dataverse_search_doi(doi):
    ...
    url = f'{dataverse}/api/datasets/:persistentId?persistentId=doi:{doi}'
    r = requests.get(url)

    try:
        r.raise_for_status()
    except requests.exceptions.HTTPError as error:
        print(f'Error looking up DOI "{doi}" in the Harvard Dataverse.')
        print(r.text)
        raise error
...
def dataverse_download_doi(doi,
    ...
    for file_metadata in metadata['data']['latestVersion']['files']:
        if requirements_match(file_metadata):
            ...
            print(f"Downloading data to '{local_fname}' ...")
