...
def clean_grbs(main_dir):
    ...
    glob_path = reduce(os.path.join,[main_dir,"*_flux","*_flux.txt"])
    ...
    for filepath in filepaths:
        ...
        num_successful += 1
        df.to_csv(os.path.join(os.path.split(filepath)[0], f"{grb}_flux_cleaned.txt"), sep='\t', index=None)
    ...
    print("="*30,"\nStats\nFiles with Duplicate Times:",num_dupes,"\nFiles with unphysical times:",len(unphysical_times["grb"]),"\nTotal GRBs:",len(filepaths),"\nNum. Successful:",num_successful,'\nTotal Points:',num_points, "\nTotal Discarded Points:",num_dupe_points,f"({100*num_dupe_points/num_points:.1f}%)")